...
lm_PHQ_FU1 <- lm(PHQ_FU1 ~ selfresilience + Age + Sex + Ethnic + BMI + site + Education, data = resilience_test)
lm_PHQ_Change <- lm(PHQ_Change ~ selfresilience + Age + Sex + Ethnic + BMI + site + Education, data = resilience_test)
lm_PHQ_Change_adj <- lm(PHQ_Change ~ PHQ_FU1 + selfresilience + Age + Sex + Ethnic + BMI + site + Education, data = resilience_test)
...
for (pair in comparisons) {
  ...
  # 5a. Linear models
  lm_model_basic <- lm(PHQ_Change ~ selfresilience + Age + Sex + Ethnic + BMI + site + Education, data = subset_data)
  lm_model_baseline <- lm(PHQ_Change ~ PHQ_FU1 + selfresilience + Age + Sex + Ethnic + BMI + site + Education, data = subset_data)
  ...
  # 2. covariates
  cov_model <- lm(PHQ_Change ~ Age + Sex + Ethnic + BMI + site + Education, data = subset_data)
  ...
  cov_model_BL <- lm(PHQ_Change ~ PHQ_FU1 + Age + Sex + Ethnic + BMI + site + Education, data = subset_data)
...
# ============================================================
# PART 2: GAD-7 longitudinal analysis (same structure)
# ============================================================

resilience_test$GAD_Change <- resilience_test$GAD_FU2 - resilience_test$GAD_FU1

lm_GAD_FU1 <- lm(GAD_FU1 ~ selfresilience + Age + Sex + Ethnic + BMI + site + Education, data = resilience_test)
lm_GAD_Change <- lm(GAD_Change ~ selfresilience + Age + Sex + Ethnic + BMI + site + Education, data = resilience_test)
lm_GAD_Change_adj <- lm(GAD_Change ~ GAD_FU1 + selfresilience + Age + Sex + Ethnic + BMI + site + Education, data = resilience_test)
...
for (pair in comparisons) {
  ...
  # 5a. Linear models
  lm_model_basic <- lm(GAD_Change ~ selfresilience + Age + Sex + Ethnic + BMI + site + Education, data = subset_data)
  lm_model_baseline <- lm(GAD_Change ~ GAD_FU1 + selfresilience + Age + Sex + Ethnic + BMI + site + Education, data = subset_data)
  ...
  # 2. covariates
  cov_model <- lm(GAD_Change ~ Age + Sex + Ethnic + BMI + site + Education, data = subset_data)
  adj_resid_cov <- resid(cov_model)
  x1_cov <- adj_resid_cov[subset_data$selfresilience == group1]
  x2_cov <- adj_resid_cov[subset_data$selfresilience == group2]
  sd_pooled_cov <- sqrt(((length(x1_cov)-1)*var(x1_cov) + (length(x2_cov)-1)*var(x2_cov)) / (length(x1_cov)+length(x2_cov)-2))
  ...
  # 3. covariates + baseline
  cov_model_BL <- lm(GAD_Change ~ GAD_FU1 + Age + Sex + Ethnic + BMI + site + Education, data = subset_data)
  adj_resid_BL <- resid(cov_model_BL)
  x1_BL <- adj_resid_BL[subset_data$selfresilience == group1]
  x2_BL <- adj_resid_BL[subset_data$selfresilience == group2]
  sd_pooled_BL <- sqrt(((length(x1_BL)-1)*var(x1_BL) + (length(x2_BL)-1)*var(x2_BL)) / (length(x1_BL)+length(x2_BL)-2))
