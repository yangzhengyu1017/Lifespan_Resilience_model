...
resilience_corr_rename_dat <- resilience_corr_rename_dat[, -1]  # remove first column
...
calculate_correlations_sex <- function(data, outcome_var, timepoint) {
  ...
  for (sex_level in c("Female", "Male")) {
    ...
    for (res_level in resilience_levels) {
      ...
      model_m <- lm(as.formula(paste(outcome_var, "~ Age + BMI + Education + Ethnic")), data = resilient_group)
      resilient_group$outcome_resid <- residuals(model_m)
      
      model_t <- lm(as.formula(paste("Trauma_exposure ~ Age + BMI + Education + Ethnic")), data = resilient_group)
