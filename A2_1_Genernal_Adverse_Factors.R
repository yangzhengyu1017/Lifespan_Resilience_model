...
for (trauma_var in names(trauma_variables_list)) {
  ...
  for (t in trauma) {
    ...
    lme1 <- lm(as.formula(paste("PHQ ~ ", t, "+ Age + Sex + Ethnic + BMI + site + Education")), data = resilience_test)
    results <- tidy(lme1)
    ...
    for (resilience_level in resilience_levels) {
      ...
      model_m <- lm(PHQ ~ Age + Sex + Ethnic + BMI + site + Education, data = resilient_group)
      ...
      model_T <- lm(as.formula(paste(t, "~ Age + Sex + Ethnic + BMI + site + Education")), data = resilient_group)
