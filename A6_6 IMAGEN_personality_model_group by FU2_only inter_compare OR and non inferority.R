...
#IMAGEN_IMGN_NEO_FFI_CHILD_RC5_IMAGEN_SURVEY_DIGEST <- read_csv("/public/mig_old_storage/home1/ISTBI_data/IMAGEN_New_Preprocessed/Behaviours/FU2/IMAGEN-IMGN_NEO_FFI_FU2-IMAGEN_SURVEY_DIGEST.csv")
...
#先按照full model的要求来选人，保证人数一致
IMAGEN_test2_dat <- IMAGEN_test_dat %>% # 创建新数据集新变量  
  transmute(eid, 
            gender,
            HH_Income_BL,
...
IMAGEN_test2_dat <- na.omit(IMAGEN_test2_dat)
IMAGEN_test1_dat <- IMAGEN_test2_dat %>% # 创建新数据集新变量  
  transmute(eid, 
            Big5_warmth, Big5_diligence, Big5_nervousness, Big5_curiosity, Big5_sociability
  )
...
BRS_train_dat[,c(2:7)] <- scale(BRS_train_dat[,c(2:7)])
IMAGEN_test1_dat[,c(2:6)] <- scale(IMAGEN_test1_dat[,c(2:6)])
...
X_colnames <- colnames(BRS_train_dat)[c(2:6)]
Y_colnames <- colnames(BRS_train_dat)[7]
...
my_plsr <- plsr(Y_train_matrix ~ X_train_matrix, ncomp=5, scale = TRUE, validation='CV')  
...
#整理IMAGEN创伤数据-------------------------
##导入16岁（FU1) 的LEQ\DAWBA\BULLY--------------
#LEQ_FU2
library(readr)
LEQ_FU2 <- read_csv("/public/mig_old_storage/home1/ISTBI_data/IMAGEN_New_Preprocessed/Behaviours/FU2/IMAGEN-IMGN_LEQ_FU2-IMAGEN_DIGEST.csv")
colnames(LEQ_FU2)[1] <- "eid"
# 删除第一列值中的最后两个字符
LEQ_FU2$eid <- substr(LEQ_FU2$eid, 1, nchar(LEQ_FU2$eid) - 2)

#LEQ_FU1
LEQ_FU1 <- read_csv("/public/mig_old_storage/home1/ISTBI_data/IMAGEN_New_Preprocessed/Behaviours/FU1/IMAGEN-IMGN_LEQ_FU_RC5-IMAGEN_DIGEST.csv")
colnames(LEQ_FU1)[1] <- "eid"
# 删除第一列值中的最后两个字符
LEQ_FU1$eid <- substr(LEQ_FU1$eid, 1, nchar(LEQ_FU1$eid) - 2)


# 获取列名
column_names <- paste0("leq_", formatC(1:39, width = 2, flag = "0"), "_ever")
LEQ_FU2 <- merge(LEQ_FU1, LEQ_FU2, by = "eid", all.y = T)

# 从第2列开始逐列进行修改
for (i in 1:39) {
  # 获取当前列名
  current_col <- column_names[i]
  # 获取对应的列名，形如leq_01_ever.x
  current_col_x <- paste0(current_col, ".x")
  corresponding_col <- paste0(current_col, ".y")
  # 修改当前列
  LEQ_FU2[[current_col_x]] <- ifelse(LEQ_FU2[[corresponding_col]] == 1, 1, LEQ_FU2[[current_col_x]])
}

LEQ_FU2 <- LEQ_FU2[,c(1:164)]
# 假设LEQ_FU2是你的数据框

# 获取列名
column_names <- names(LEQ_FU2)

# 替换列名中包含ever.x的部分
new_column_names <- gsub("\\.x$", "", column_names)

# 将数据框的列名替换为新的列名
names(LEQ_FU2) <- new_column_names
...
#合并创伤数据进行处理----------------------
IMAGEN_trauma <- merge(LEQ_FU2, IMAGEN_FU2_CTQ, by = "eid", all.x =T)
...
summary_data <- trauma_dat %>%
  ...
  summarize(diag_prop = mean(AnyDisorder_FU2))
...
trauma_dat <- trauma_dat %>%
  mutate(Diag_resilience = ifelse(AnyDisorder_BL == 0 & AnyDisorder_FU1 == 0 & AnyDisorder_FU2 == 0 & trauma == 0, 0,
                                  ifelse(AnyDisorder_BL == 1 |AnyDisorder_FU1 == 1 | AnyDisorder_FU2 == 1, 2,
                                         ifelse(AnyDisorder_BL == 0 & AnyDisorder_FU1 == 0 & AnyDisorder_FU2 == 0 & trauma == 1, 1, NA))))

#trauma_dat <- trauma_dat %>%
#  mutate(Diag_resilience = ifelse(Internalizing_BL == 0 & Internalizing_FU1 == 0 & Internalizing_FU2 == 0 & trauma == 0, 0,
#                                  ifelse(Internalizing_BL == 1 |Internalizing_FU1 == 1 | Internalizing_FU2 == 1, 2,
#                                         ifelse(Internalizing_BL == 0 & Internalizing_FU1 == 0 & Internalizing_FU2 == 0 & trauma == 1, 1, NA))))
...
trauma_dat <- trauma_dat %>%
  mutate(Model_resilience = ifelse(AnyDisorder_BL == 0 & AnyDisorder_FU1 == 0 & AnyDisorder_FU2 == 0 & trauma == 0, 0,
                                   ifelse(AnyDisorder_BL == 1 | AnyDisorder_FU1 == 1 | AnyDisorder_FU2 == 1, 3,
                                          ifelse(AnyDisorder_BL == 0 & AnyDisorder_FU1 == 0 & AnyDisorder_FU2 == 0 & pred_resilience_IMAGEN >= percentile_high$percentile_high & trauma == 1, 1,
                                                 ifelse(AnyDisorder_BL == 0 & AnyDisorder_FU1 == 0 & AnyDisorder_FU2 == 0 & pred_resilience_IMAGEN < percentile_low$percentile_low & trauma == 1, 2, NA)))))
...
p1 <- ggplot(long_data, aes(x = Variable, y = Value, group = Diag_resilience, color = Diag_resilience)) +
  # 添加灰色背景矩形
  geom_rect(aes(xmin = 0, xmax = 3.5, ymin = -Inf, ymax = Inf), fill = 'grey90', color = NA) +
  # 添加竖直虚线
  geom_vline(xintercept = 3.5, linetype = "dashed", size = 0.9) +
  # 绘制每组的均值线
...
results <- results %>%
  add_row(
    ...
    CI_Upper = 1,
    P_Value = NA,  # 基线没有 P 值
    Compare = "Diag_resilience-Low_risk"
  )
results <- results %>%
  add_row(
    Time_Point = "Internalizing_FU1",
    OR = 1,
    CI_Lower = 1,
    CI_Upper = 1,
    P_Value = NA,  # 基线没有 P 值
...
# 循环计算 OR 和 P 值
for (time in colnames(DAWBA_FU1_result)[4]) {
  # 提取子集数据并重命名 outcome
...
results <- results %>%
  add_row(
    ...
    CI_Upper = 1,
    P_Value = NA,  # 基线没有 P 值
    Compare = "Model_resilience-Low_risk"
  )
results <- results %>%
  add_row(
    Time_Point = "Internalizing_FU1",
...
ggplot(or_results_plot, aes(x = Time_Point, y = OR, fill = Compare)) +
  ...
  annotate("segment", x = 3.5, xend = Inf, y = 2.3, yend = 2.3, linetype = "dashed", color = "red", size = 0.9) +
...
ggplot(or_results_plot, aes(x = Time_Point, y = OR, color = Compare)) +
  # 添加灰色背景矩形
  geom_rect(aes(xmin = 1.5, xmax = Inf, ymin = -Inf, ymax = Inf), fill = 'grey90', color = NA) +
  # 添加竖直虚线
  ...
  scale_y_continuous(limits = c(0, 10)) +  # 限制 y 轴刻度为 0 到 8
  labs(x = "Time Point", y = "Odds Ratio (OR)", fill = "Group") +  # Add labels
  theme_classic() +  # Minimal theme
