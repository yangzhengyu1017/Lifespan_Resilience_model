...
#ESPAD中的BULLY数据
IMAGEN_FU1_ESPAD <- read_csv("/public/mig_old_storage/home1/ISTBI_data/IMAGEN_New_Preprocessed/Behaviours/FU1/IMAGEN-IMGN_ESPAD_CHILD_FU_RC5-IMAGEN_DIGEST.csv")
colnames(IMAGEN_FU1_ESPAD)[1] <- "eid"
IMAGEN_FU1_ESPAD$eid <- substr(IMAGEN_FU1_ESPAD$eid, 1, nchar(IMAGEN_FU1_ESPAD$eid) - 2)

#合并创伤数据进行处理----------------------
IMAGEN_trauma <- merge(LEQ_FU1, IMAGEN_FU2_CTQ, by = "eid", all.x =T)
IMAGEN_trauma <- merge(IMAGEN_trauma, IMAGEN_FU1_DAWBA, by = "eid", all.x =T)
IMAGEN_trauma <- merge(IMAGEN_trauma, IMAGEN_FU1_ESPAD, by = "eid", all.x =T)


IMAGEN_trauma_selected <- IMAGEN_trauma %>%
  transmute(eid, Gender = gender, Age_FU1 = age,
...
trauma_dat <- trauma_dat %>%
  mutate(Diag_resilience = ifelse(AnyDisorder_BL == 0 & AnyDisorder_FU1 == 0 & trauma == 0, 0,
                                  ifelse(AnyDisorder_BL == 1 | AnyDisorder_FU1 == 1, 2,
                                         ifelse(AnyDisorder_BL == 0 & AnyDisorder_FU1 == 0 & trauma == 1, 1, NA))))
...
trauma_dat <- trauma_dat %>%
  mutate(Model_resilience = ifelse(AnyDisorder_BL == 0 & AnyDisorder_FU1 == 0 & trauma == 0, 0,
                                   ifelse(AnyDisorder_BL == 1 | AnyDisorder_FU1 == 1, 3,
                                          ifelse(AnyDisorder_BL == 0 & AnyDisorder_FU1 == 0 & pred_resilience_IMAGEN >= percentile_high$percentile_high & trauma == 1, 1,
                                                 ifelse(AnyDisorder_BL == 0 & AnyDisorder_FU1 == 0 & pred_resilience_IMAGEN < percentile_low$percentile_low & trauma == 1, 2, NA)))))
...
p1 <- ggplot(long_data, aes(x = Variable, y = Value, group = Diag_resilience, color = Diag_resilience)) +
  # 添加灰色背景矩形
  geom_rect(aes(xmin = 0, xmax = 2.5, ymin = -Inf, ymax = Inf), fill = 'grey90', color = NA) +
  # 添加竖直虚线
  geom_vline(xintercept = 2.5, linetype = "dashed", size = 0.9) +
  # 绘制每组的均值线
...
# 循环计算 OR 和 P 值
for (time in colnames(DAWBA_FU1_result)[3:4]) {
  # 提取子集数据并重命名 outcome
...
ggplot(or_results_plot, aes(x = Time_Point, y = OR, fill = Compare)) +
  ...
  annotate("segment", x = 2.5, xend = Inf, y = 2.3, yend = 2.3, linetype = "dashed", color = "red", size = 0.9) +
...
ggplot(or_results_plot, aes(x = Time_Point, y = OR, color = Compare)) +
  # 添加灰色背景矩形
  geom_rect(aes(xmin = 2.5, xmax = Inf, ymin = -Inf, ymax = Inf), fill = 'grey90', color = NA) +
  # 添加竖直虚线
