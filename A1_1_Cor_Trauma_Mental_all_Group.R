...
# Load data
# -------------------------------
resilience_corr_rename_dat <- read_csv(
  "~/Documents/ongoing_project/child_maltreatment_UKB/Resilience_modle/UKB_resilience_corr_rename_dat_11221.csv"
)[,-1]

soc_env_trauma_mental_dat <- read.csv(
  "~/Documents/ongoing_project/child_maltreatment_UKB/Resilience_modle/UKB_BL_FU_trauma_mental_env_dat.csv"
)[,-1]

# -------------------------------
# Merge datasets
# -------------------------------
# Only select relevant columns from trauma/mental/environment dataset
resilience_group_R <- merge(
  resilience_corr_rename_dat,
  soc_env_trauma_mental_dat[, c(1,71,85,79)],
  by = "eid", all.x = TRUE
)
...
resilience_group_R <- resilience_group_R %>%
  mutate(
    # Normalize self-resilience to a 0-1 scale
    self_resilience = (self_resilience + 6) / 6,
    
    # Scale FU2 recent trauma
    FU2_recent_trauma = FU2_recent_trauma * 10,
    
    # Recode ethnicity categories
    ...
    # Shift categories to start from 0
